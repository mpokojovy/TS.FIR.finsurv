%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   A Fast Initial Response Approach to Real-Time Financial Surveillance  %
%            (C) Michael Pokojovy and Andrews T. Anum (2022)              %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function run_control_charts(dates, x, CUSUM_k, alpha, all_ok_ARL)
    n = length(x);

    % Initialize signal vectors
    TS_SIGNALS    = zeros(1, n);
    CUSUM_L_NO_HS = zeros(1, n);
    CUSUM_U_NO_HS = zeros(1, n);
    CUSUM_L_HS    = zeros(1, n);
    CUSUM_U_HS    = zeros(1, n);

    % Load up the TS chart control limit

    cd '../util/';
    CL_TS = get_TS_statistic(alpha, all_ok_ARL, 'ControlLimitConservative');
    cd '../examples/';

    % Initialize CUSUM parameters/control limit

    % Set of k values
    K = [0.1000  0.2000  0.2500  0.3000  0.4000  0.5000  0.6000  0.7000  0.7500  0.8000  0.9000  1.0000  1.1000  1.2000  1.2500  1.3000  1.4000  1.5000]';
    % Set of control limits h for CUSUM
    H = ...
        [ 8.5200    6.3200    5.6000    5.0100    4.1400    3.5000    3.0200    2.6400    2.4820    2.3370    2.0850    1.8740    1.6920    1.5320    1.4580    1.3880    1.2550    1.1320;
         13.4860    9.2490    8.0100    7.0630    5.7070    4.7740    4.0882    3.5600    3.3400    3.1408    2.7988    2.5162    2.2793    2.0770    1.9860    1.9012    1.7450    1.6042;
         16.5475   10.9238    9.3700    8.2085    6.5750    5.4720    4.6715    4.0616    3.8075    3.5800    3.1891    2.8662    2.5959    2.3671    2.2655    2.1705    1.9988    1.8415]';
    % Set of control limits h for CUSUM head start
    H_HS = ...
        [ 9.8129    6.8781    5.9994    5.3201    4.3298    3.6357    3.1179    2.7153    2.5458    2.3933    2.1298    1.9093    1.7204    1.5547    1.4786    1.4062    1.2703    1.1438;
         14.6162    9.6477    8.2854    7.2663    5.8298    4.8560    4.1468    3.6046    3.3781    3.1750    2.8263    2.5383    2.2970    2.0914    1.9992    1.9129    1.7549    1.6121;
         17.5166   11.2407    9.5854    8.3644    6.6676    5.5332    4.7150    4.0942    3.8362    3.6053    3.2094    2.8826    2.6093    2.3780    2.2751    2.1794    2.0061    1.8518]';   

     % Index of the (k, h)-pair
    ind = find(K == CUSUM_k);
    k = K(ind);

    switch all_ok_ARL
        case 100
            h    = H(ind, 1);
            h_HS = H_HS(ind, 1);
        case 370
            h    = H(ind, 2);
            h_HS = H_HS(ind, 2);
        case 750
            h    = H(ind, 3);
            h_HS = H_HS(ind, 3);
    end

    %% Signal computation
    % CUSUM chart w/o head start
    L = 0;
    U = 0;

    for m = 1:n
        L = min([0 (x(m) + k) + L]);
        U = max([0 (x(m) - k) + U]);

        CUSUM_L_NO_HS(m) = L;
        CUSUM_U_NO_HS(m) = U;

        if (L < -h) || (U > h)
            L = 0;
            U = 0;
        end
    end

    % CUSUM chart w/ head start
    L = -h/2;
    U =  h/2;

    for m = 1:n
        L = min([0 (x(m) + k) + L]);
        U = max([0 (x(m) - k) + U]);

        CUSUM_L_HS(m) = L;
        CUSUM_U_HS(m) = U;

        if (L < -h_HS) || (U > h_HS)
            L = -h_HS/2;
            U =  h_HS/2;
        end
    end

    % TS chart
    cd '../lib/';

    i0 = 1;
    for m = 1:n
        [TS_SIGNALS(m), ~] = TSChartSingleStep(x(i0:m), 0, 1, alpha);

        if (TS_SIGNALS(m) > CL_TS)
            i0 = m + 1;
        end
    end
    
    %% Plotting
    figure;
    
    set(gcf, 'PaperUnits', 'centimeters');
    xSize = 28; ySize = 14;
    xLeft = (21 - xSize)/2; yTop = (30 - ySize)/2;
    set(gcf,'PaperPosition', [xLeft yTop xSize ySize]);
    set(gcf,'Position', [0 0 xSize*50 ySize*50]);

    color_order = get(gca, 'colororder');
    %% TS
    subplot_tight(1, 3, 1, [0.08 0.05]);

    hold on;
    p1 = plot(dates, TS_SIGNALS,       '-',  'Color', color_order(1, :));
    p2 = plot(dates, ones(n, 1)*CL_TS, '-.', 'Color', color_order(1, :));
    p3 = plot(dates, TS_SIGNALS,       'o',  'Color', color_order(1, :));

    xlabel('Date', 'interpreter', 'latex', 'FontSize', 18);
    ylabel('TS signal', 'interpreter', 'latex', 'FontSize', 18);

    % TS detection time
    IND = find(TS_SIGNALS > CL_TS);

    LY = -2*max(TS_SIGNALS) - 1;
    
    for i = 1:1
        plot([dates(IND(i)) dates(IND(i))], [LY TS_SIGNALS(IND(i))], 'k--', dates(IND(i)), LY, 'k*');
    end

    ax = gca;

    ylim([0.0 max(TS_SIGNALS)*1.4]);

    legend([p3, p2], {'TS signals', 'Control limit'}, ...
            'Location', 'Best', 'interpreter', 'latex', 'FontSize', 18);

    %% CUSUM FIR
    subplot_tight(1, 3, 2, [0.08 0.05]);
    hold on;

    p1a = plot(dates, CUSUM_L_HS,    '-',  'Color', color_order(2, :));
    p1b = plot(dates, CUSUM_U_HS,    '-',  'Color', color_order(2, :));

    p2a = plot(dates,  h_HS*ones(1, n), '-.', 'Color', color_order(2, :));
    p2b = plot(dates, -h_HS*ones(1, n), '-.', 'Color', color_order(2, :));

    p3a = plot(dates, CUSUM_L_HS,    'p', 'Color', color_order(2, :), 'MarkerEdgeColor', color_order(2, :), 'MarkerFaceColor', color_order(2, :));
    p3b = plot(dates, CUSUM_U_HS,    'p', 'Color', color_order(2, :), 'MarkerEdgeColor', color_order(2, :), 'MarkerFaceColor', color_order(2, :));

    % CUSUM head start detection time
    [~, I1] = find(CUSUM_L_HS < -h_HS);
    [~, I2] = find(CUSUM_U_HS >  h_HS);

    LY = -2*(max(abs(CUSUM_L_HS)) + max(abs(CUSUM_U_HS))) - 1;
    
    for i = 1:min(1, length(I1))
        plot(dates([I1(i) I1(i)]), [LY CUSUM_L_HS(I1(i))], 'k--', dates(I1(i)), LY, 'k*');
    end

    for i = 1:min(1, length(I2))
        plot(dates([I2(i) I2(i)]), [LY CUSUM_U_HS(I2(i))], 'k--', dates(I2(i)), LY, 'k*');
    end

    legend([p3a, p2a], {['CUSUM($k$ = ', num2str(k), ') FIR signals'], 'CUSUM FIR control limits'}, ...
            'Location', 'North', 'interpreter', 'latex', 'FontSize', 16);

    xlabel('Date', 'interpreter', 'latex', 'FontSize', 18);
    ylabel('CUSUM FIR signal', 'interpreter', 'latex', 'FontSize', 18);

    ax = gca;

    LY = 1.5*max([abs(CUSUM_L_HS) abs(CUSUM_U_HS)]);
    
    ylim([-LY LY]);

    %% CUSUM

    subplot_tight(1, 3, 3, [0.08 0.05]);
    hold on;

    p1a = plot(dates, CUSUM_L_NO_HS,    '-',  'Color', color_order(5, :));
    p1b = plot(dates, CUSUM_U_NO_HS,    '-',  'Color', color_order(5, :));

    p2a = plot(dates,  h*ones(1, n), '-.', 'Color', color_order(5, :));
    p2b = plot(dates, -h*ones(1, n), '-.', 'Color', color_order(5, :));

    p3a = plot(dates, CUSUM_L_NO_HS,    'p', 'Color', color_order(5, :), 'MarkerEdgeColor', color_order(5, :), 'MarkerFaceColor', color_order(5, :));
    p3b = plot(dates, CUSUM_U_NO_HS,    'p', 'Color', color_order(5, :), 'MarkerEdgeColor', color_order(5, :), 'MarkerFaceColor', color_order(5, :));

    % CUSUM head start detection time
    [~, I1] = find(CUSUM_L_NO_HS < -h_HS);
    [~, I2] = find(CUSUM_U_NO_HS >  h_HS);

    LY = -2*(max(abs(CUSUM_L_NO_HS)) + max(abs(CUSUM_U_NO_HS))) - 1;
    
    for i = 1:min(1, length(I1))
        plot(dates([I1(i) I1(i)]), [LY CUSUM_L_NO_HS(I1(i))], 'k--', dates(I1(i)), LY, 'k*');
    end

    for i = 1:min(1, length(I2))
        plot(dates([I2(i) I2(i)]), [LY CUSUM_U_NO_HS(I2(i))], 'k--', dates(I2(i)), LY, 'k*');
    end

    legend([p3a, p2a], {['CUSUM($k$ = ', num2str(k), ') signals'], 'CUSUM control limits'}, ...
            'Location', 'North', 'interpreter', 'latex', 'FontSize', 17);

    xlabel('Date', 'interpreter', 'latex', 'FontSize', 18);
    ylabel('CUSUM FIR signal', 'interpreter', 'latex', 'FontSize', 18);

    ax = gca;
    
    LY = 1.5*max([abs(CUSUM_L_NO_HS) abs(CUSUM_U_NO_HS)]);

    ylim([-LY LY]);

    %%
    cd '../FS.stock.example'/;    
end